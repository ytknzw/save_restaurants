...
from const import URL_TEST, URL_PROD, CLIENT_SECRET_FILE, SENDER_ADDRESS, TO_ADDRESS, SUBJECT, TEXT


def main():
    for url in [URL_TEST, URL_PROD]:
        try:
            now, file_path = run_link_check(url)
        except:
            ...
            try:
                ...
                message = create_message_with_attachment(sender=SENDER_ADDRESS, to=TO_ADDRESS,
                                                         subject=SUBJECT, message_text=f"{url}\n{now}\n{TEXT}",
                                                         file=file_path)
